...
| policy input | cumulative residual path over the 30-day window, x 100 (`policy.input: cumulative`, `input_scale`) | GPZ's `preprocess_cumsum`, i.e. the two-step benchmark's own convention; the paper's formula LongConv(eps_{t-s..t-1}) is `policy.input: returns`. A price-level signal changes more slowly than a return signal, which is the whole cost story (grid below). The runs below use percent units. The L1-normalised output is NOT invariant to that scale: the encoder bias, GELU and the GLU gate make the learned policy depend on it, and for the attention model decimal units traded a third less (`configs/us_replication.yaml`), so this row has to be rerun at `input_scale: 1.0` before the Table 2 comparison |
